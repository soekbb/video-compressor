...
跨平台桌面视频工具（Windows / macOS）。支持批量压制与视频合成；压制在**不改变分辨率**的前提下压缩体积，输出文件名自动加 `_batch` 后缀。
...
- **桌面壳**：Tauri 2
- **压缩引擎**：内置 FFmpeg sidecar（`libx264` CRF 18，画质优先，不缩放分辨率）；安装包自带，用户无需再装 FFmpeg
